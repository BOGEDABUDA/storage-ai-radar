...
- **两类真实数据稀疏（非解析缺陷，UI 与趋势计算必须按「真实 0」处理，不可当缺失值）**：
  - 42 期各有 10 个分类文件，但其中 **12 期存在分类为 0 篇**（文件仅 47–57 字节，只有标题行）。最稀疏的 `2026-05-15` 仅 22 篇文章、覆盖 5 个分类
  - **32 篇文章的「总结」在源数据中即为空**（多为招聘/汽车等非科技内容，总结环节未产出）
...
| **P2** | 实体抽取（LLM+缓存）+ 知识图谱页 + 趋势跟踪页 | ✅ 已完成（含 3 处正确性修正，见 README「P2 相对设计文档的修正」） |
| **P3** | Agent 服务（FTS5 RAG + SSE + 引用）+ 站点侧栏与降级 | ✅ 已完成（问答带引用；Agent 未启动时站点功能不受影响，已实测） |

| **P4** | 文章去重 + 学术论文推荐（arXiv 解析与链接） | ✅ 已完成（追加需求） |
