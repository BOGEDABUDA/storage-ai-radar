...
| 3. 归并 | 同一篇论文多期被反复推荐（最多 6 次）→ 合并成一条，日期/领域取并集 | 927 篇唯一论文，其中 513 篇匹配到 arXiv |
...
一条命令搞定：`./publish.sh`（自动检测新期数 → 增量分析 → 构建 → 校验 → 推送 → 验证线上，实测约 1 分 20 秒；arXiv 解析限速另计 2–4 分钟）。

| 命令 | 用途 |
|---|---|
| `./publish.sh` | 完整发布（含推送与线上验证） |
| `./publish.sh --check` | 只看有没有新内容 + 口径自检，不做改动 |
| `./publish.sh --dry-run` | 打印将要执行的步骤 |
| `./publish.sh --no-push` | 只构建到本地，自己预览后再推 |
| `./publish.sh --skip-arxiv` | 跳过 arXiv 解析（快路径） |

**按内容检测而非按时间**：日报完成时间在 07:57–11:33 之间漂移，偶尔滑到周二/周五，定时触发一定会漏。
只认 `output/<date>/daily_report.md` 已存在且稳定 90 秒的期，避免发布半成品。

**验证门禁**：只读断言 + 本地自检（6 个端点 + 新期数必须真的进产物）任一不过就中止，绝不推送。
重复运行不会产生噪音提交（会忽略纯 `generated_at` 差异）。

## 分析口径：哪些需要同步，哪些只是旋钮

| 文件 | 性质 | 需要同步吗 |
|---|---|---|
| `pipeline/categories.py` | 语料分类名 ↔ URL slug 的**契约** | **需要**：日报那 10 个分类增删/改名时必须更新 |
| `pipeline/aliases.json` | 实体别名归一（80 组 / 171 变体） | 不需要，是**质量旋钮**：决定「英伟达」与「NVIDIA」在图谱里是否合并 |
| `pipeline/analyze_trends.py` 顶部常量 | 动量窗口、入图阈值等 | 不需要，是**分析口径选择**，没有对错 |

`categories.py` 不同步不会崩（未登记分类会退化为 `cat-<hash>` slug），但 URL 难看且丢配色与排序。

**这件事已经自动化**：`publish.sh` 每次运行都会做「口径自检」，对比语料分类与 `categories.py`，
不一致就明确列出该补什么；同时报告别名覆盖率。所以你不需要记着什么时候该同步。

改了别名或趋势常量后**不需要重跑 LLM**，零 API 成本：

```bash
python3 -m pipeline.analyze_trends   # 只重算图谱与趋势
python3 -m pipeline.build            # 重算全部派生数据
```
